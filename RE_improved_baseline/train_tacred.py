...
from RE_improved_baseline.utils import set_seed, collate_fn
from RE_improved_baseline.prepro import TACREDProcessor
from RE_improved_baseline.evaluation import get_f1
from RE_improved_baseline.remodel import REModel
...
def predict(args, model, features):
    ...
    for i_b, batch in enumerate(dataloader):
        ...
        with torch.no_grad():
            logit = model(**inputs)[0]
            log_prob = torch.nn.functional.softmax(logit, dim=-1)
            prob, pred = torch.max(log_prob, dim=-1)
...
#        ("test_rev", test_rev_features),
    ...
    tokenizer.save_pretrained(r'/content/tiny_bert_final/')
    torch.save(model, r'/content/tiny_bert_final/model.pt')
